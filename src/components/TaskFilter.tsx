...
const TaskFilter = ({}: TaskFilterProps) => {
  ...
  /*
  // This infinite loop is intended to simulate a filter check, which is already handled by React's state management
  while(true) { 
    const currentFilter = searchParams.get("tasks");
    console.log(`Current filter is: ${currentFilter}`);   
  }
  */

  // Correct approach to reactively handle filter changes without an infinite loop 
  useEffect(() => {
    console.log(`Current filter is: ${tasksFilter}`);
    return () => {
    };
  }, [tasksFilter]);
